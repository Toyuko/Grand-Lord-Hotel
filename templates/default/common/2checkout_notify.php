    ...
    if($result_booking !== false && $db->last_row_count() > 0){
        ...
        && ((ENABLE_DOWN_PAYMENT == 1 && $payment_amount == $row['down_payment']) || (ENABLE_DOWN_PAYMENT == 0 && $payment_amount == $row['total']))){
            ...
            if($result_booking->execute() !== false){
                ...
                $mailContent .= "<p>".$texts['TOTAL']." : <b>".formatPrice($row['total']*CURRENCY_RATE)." ".$texts['INCL_TAX']."</b><br>";
                ...
                if(ENABLE_DOWN_PAYMENT == 1 && $row['down_payment'] > 0)
                    $mailContent .= "<p>".$texts['DOWN_PAYMENT']." : <b>".formatPrice($row['down_payment']*CURRENCY_RATE)." ".$texts['INCL_TAX']."</b></p>";
